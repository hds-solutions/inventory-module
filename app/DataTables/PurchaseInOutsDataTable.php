...
class PurchaseInOutsDataTable extends Base\DataTable {
    ...
    protected function getColumns() {
        return [
            ...
            Column::make('invoice.document_number')
                ->title( __('inventory::in_out.invoice_id.0') ),
    ...
    protected function joins(Builder $query):Builder {
        // load Purchaes InOuts only
        // add custom JOIN to providers + people for Partnerable
        return $query->isPurchase()
            // join to partnerable
            ->leftJoin('providers', 'providers.id', 'in_outs.partnerable_id')
            // join to people
