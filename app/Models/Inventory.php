...
class Inventory extends X_Inventory implements Document {
    ...
    public function prepareIt():?string {
        ...
        if (!$this->lines->count()) return $this->documentError( __('models/inventory.no-lines') );
        // foreach lines
        foreach ($this->lines as $line)
            // check if line has current qty set
            if ($line->counted === null)
                // return error
                return $this->documentError( __('models/inventory.line.empty-counted') );
    ...
    public function completeIt():?string {
        ...
        if (!$this->isApproved()) return $this->documentError( __('models/inventory.not-approved') );
        // foreach lines
        foreach ($this->lines as $line) {
